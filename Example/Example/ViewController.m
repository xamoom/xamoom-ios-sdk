...
- (void)contentWithID {
    [self.api contentWithID:@"7cf2c58e6d374ce3888c32eb80be53b5" password:nil completion:^(XMMContent *content, NSError *error, BOOL passwordRequired) {
        if (error) {
            NSLog(@"Error: %@", error);
            return;
        }
...
- (void)contentWithIDOptions {
    [self.api contentWithID:@"7cf2c58e6d374ce3888c32eb80be53b5" options:XMMContentOptionsPrivate password:nil completion:^(XMMContent *content, NSError *error, BOOL passwordRequired) {
        if (error) {
            NSLog(@"Error: %@", error);
            return;
        }
...
- (void)contentWithLocationIdentifier {
    [self.api contentWithLocationIdentifier:@"b5v2p" password:nil completion:^(XMMContent *content, NSError *error, BOOL passwordRequired) {
        if (error) {
            NSLog(@"Error: %@", error);
            return;
        }
...
- (void)contentWithBeaconMajor {
    [self.api contentWithBeaconMajor:@54222 minor:@24265 completion:^(XMMContent *content, NSError *error, BOOL passwordRequired) {
        if (error) {
            NSLog(@"Error: %@", error);
            return;
        }
...
- (void)contentWithTags {
    self.api.offline = NO;
    [self.api contentsWithTags:@[@"x-start"] pageSize:10 cursor:nil sort:0 completion:^(NSArray *contents, bool hasMore, NSString *cursor, NSError *error) {
        if (error) {
            NSLog(@"Error: %@", error);
            return;
        }
        ...
        for (XMMContent *content in contents) {
            [self.api contentWithID:content.ID password:nil completion:^(XMMContent *content2, NSError *error, BOOL passwordRequired) {
                [self.blocks displayContent:content];
...
- (void)loadSystem {
    [self.api systemWithCompletion:^(XMMSystem *system, NSError *error) {
        ...
        if (!system) {
            NSLog(@"Error: System returned is nil.");
            return;
        }
